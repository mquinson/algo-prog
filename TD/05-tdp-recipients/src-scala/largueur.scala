...
def egal(src:Array[Int], dst:Array[Int]) {
    for (i <- 0 to src.length-1)
        if (src(i) != dst(i))
            return false;
    return true;
}
def contient(liste:Liste[Array[Int]], elm:Array[Int]) {
    for (e <- liste) 
        if (egal(e,elm))
            return true;
    return false;
...
def cherche(capa:Array[Int], cible:Int):Boolean = {
    ...
    def lambda(rang:Int, dejaVues:List[Array[Int]]):Boolean= {   
        println("Profondeur "+rang+". Nombres de solutions pour l'instant: "+(dejaVues.size))
        var nextVues:List[Array[Int]] = Nil;
        for (old <- dejaVues) {
            for (src <- 0 to NB_RECIPIENTS) {     // on choisit un recipient source 
                for (dst <- 0 to NB_RECIPIENTS) { // et un recipient cible ... 
                    ...
                    if (faitQQCH && !contient(nextVues,copy)) { 
                        affiche("Pas bon. ",copy,"");
                        nextVues = copy :: nextVues;
                    }
                }
            }
        }
        return lambda(rang+1, nextVues)
    }   
    ...
    return lambda(0, Array.fill(capa.length)(0):: Nil)
}
// END CHERCHE

//cherche(Array(5,7), 4); // L'exemple de l'énoncé

 cherche(Array(100,24,25), 6)
