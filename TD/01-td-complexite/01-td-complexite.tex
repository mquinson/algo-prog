...
\begin{table}[h]
  ...
  \subfigure[Plus grande instance faisable à $10^9$ op/sec.]{
    \begin{tabular}{|c|c|c|c|}\hline
      ...
      $n^2$&\trou{$\sqrt{10^9}=3\cdot 10^4$}&\trou{$10^6$}&\trou{$10^8$}\\\hline
...
\begin{Reponse}
  1s=$10^9$ opérations. Donc l'algo n fait $10^9$ opérations,
  $n^2\leadsto\sqrt(10^9)$, $2^n\leadsto\log_2(10^9)$ , $\log n\leadsto
  2^{10^9}$

  1h, c'est idem $\times$ 3600. En gros, on calcule le nb d'op sur la première
  ligne, et après on extrait des racines carrés, des racines cubiques, des logs
  et des exp.
